...
nom = [] #Liste qui contient le nom de chaque joueur
score = [] #Liste qui contient le score de chaque joueur
...
def creation_plateau(canvas):
    ...
    l_offset = 37#286+36 #Largeur à partir du bord de la fenêtre
    h_offset = 42#42 #Hauteur à partir du bord de la fenêtre
...
w1 = c.create_window(l_jeu//2, h_jeu//2)
c1 = tk.Canvas(win, width = 708, height = h_jeu)
c.itemconfigure(w1, window = c1)

#Placement des images
fond = creation_objet(c, "image", img_fond, l_jeu//2, h_jeu//2)
plateau = creation_objet(c1, "image", img_plateau, 712//2, h_jeu//2)
creation_plateau(c1)
...
c.itemconfigure(w1, state = "hidden")
